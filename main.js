...
var STATUS_EMOJI = {
  "want-to-read": "\u{1F4DA}",
  reading: "\u{1F4D6}",
  finished: "\u2705",
  abandoned: "\u274C"
};
var LIBRARY_VIEW_TYPE = "book-tracker-library";
var LibraryView = class extends import_obsidian.ItemView {
  ...
  async render() {
    ...
    const allStatuses = ["all", "reading", "finished", "want-to-read", "abandoned"];
    ...
    const books = this.plugin.app.vault.getMarkdownFiles().filter((f) => f.path.startsWith(this.plugin.settings.booksFolder + "/") && !f.path.startsWith(this.plugin.settings.themesFolder + "/"));
    const filtered = this.filterStatus === "all" ? books : books.filter((f) => {
      var _a2;
      const cache = this.plugin.app.metadataCache.getFileCache(f);
      return ((_a2 = cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a2.status) === this.filterStatus;
    });
    ...
    for (const file of filtered.sort((a, b) => b.stat.mtime - a.stat.mtime)) {
      ...
      const status = (_b = fm.status) != null ? _b : "want-to-read";
      const rating = (_c = fm.rating) != null ? _c : 0;
      ...
      if (fm.author)
        card.createEl("div", { text: fm.author, cls: "bt-card-author" });
      if (fm.genre)
        card.createEl("div", { text: fm.genre, cls: "bt-card-genre" });
    }
  }
};
var AddBookModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).setName("Author").addText((t) => t.setPlaceholder("Author name").onChange((v) => data.author = v));
    new import_obsidian.Setting(contentEl).setName("Genre").addText((t) => t.setPlaceholder("e.g. Fiction, Biography").onChange((v) => data.genre = v));
    new import_obsidian.Setting(contentEl).setName("Year published").addText((t) => t.setPlaceholder("e.g. 2021").onChange((v) => data.year = v));
    new import_obsidian.Setting(contentEl).setName("Pages").addText((t) => t.setPlaceholder("e.g. 320").onChange((v) => data.pages = v));
    new import_obsidian.Setting(contentEl).setName("Status").addDropdown((d) => {
      ...
      d.setValue("want-to-read").onChange((v) => data.status = v);
    });
    new import_obsidian.Setting(contentEl).setName("Rating (0\u20135)").addSlider(
      (s) => s.setLimits(0, 5, 1).setValue(0).setDynamicTooltip().onChange((v) => data.rating = v)
    );
    new import_obsidian.Setting(contentEl).setName("Review").addTextArea((a) => {
      a.setPlaceholder("Your thoughts...").onChange((v) => data.review = v);
      a.inputEl.rows = 3;
      a.inputEl.addClass("bt-textarea");
    });
    new import_obsidian.Setting(contentEl).setName("Favourite quotes").addTextArea((a) => {
      a.setPlaceholder("One quote per line...").onChange((v) => data.quotes = v);
      a.inputEl.rows = 3;
      a.inputEl.addClass("bt-textarea");
    });
    new import_obsidian.Setting(contentEl).setName("Themes (comma-separated)").addText(
      (t) => t.setPlaceholder("e.g. identity, power, memory").onChange((v) => data.themes = v)
    );
    new import_obsidian.Setting(contentEl).addButton(
      (btn) => btn.setButtonText("Add Book").setCta().onClick(async () => {
        if (!data.title.trim()) {
...
var BookTrackerPlugin = class extends import_obsidian.Plugin {
  async onload() {
  ...
  openAddBookModal(onDone) {
    new AddBookModal(this.app, this, onDone != null ? onDone : () => {
    }).open();
  }
  ...
  async createBook(data) {
    ...
    const themeLinks = data.themes.split(",").map((t) => t.trim()).filter(Boolean).map((t) => `[[${t}]]`).join(", ");
    const body = `---
title: "${data.title}"
author: "${data.author}"
genre: "${data.genre}"
...
`);
    ...
    new import_obsidian.Notice(`Added: ${data.title}`);
    return file;
  }
  plugin_settings() {
...
var BookTrackerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Books folder").addText(
      (t) => t.setValue(this.plugin.settings.booksFolder).onChange(async (v) => {
        this.plugin.settings.booksFolder = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Themes folder").addText(
      (t) => t.setValue(this.plugin.settings.themesFolder).onChange(async (v) => {
        this.plugin.settings.themesFolder = v;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Auto-open new book notes").addToggle(
      (t) => t.setValue(this.plugin.settings.autoOpenBook).onChange(async (v) => {
